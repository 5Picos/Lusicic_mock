...
import { clients as initialItems } from '@/lib/mock-data'
...
export default function ClientesPage() {
  ...
  function handleSave() {
    ...
    if (editingItem) {
      ...
      setItems(prev => [...prev, { id: `cl${Date.now()}`, ...form, name: form.name.trim() }])
    }
    setSheetOpen(false)
  }
  ...
  return (
    <div>
      <PageHeader
        title="Clientes"
        action={<Button size="sm" onClick={openNew}><Plus size={14} className="mr-1" /> Nuevo cliente</Button>}
      />
      <DataTable
        columns={columns}
        rows={filtered}
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="w-[400px] flex flex-col">
          <SheetHeader><SheetTitle>{editingItem ? 'Editar cliente' : 'Nuevo cliente'}</SheetTitle></SheetHeader>
          <div className="flex flex-col gap-4 p-4 flex-1">
            <div className="flex flex-col gap-1.5">
              <Label className="text-[10px] uppercase text-slate-500">Nombre</Label>
              <Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Molinos Río de la Plata" autoFocus />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label className="text-[10px] uppercase text-slate-500">Teléfono</Label>
              <Input value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} placeholder="1130001111" />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label className="text-[10px] uppercase text-slate-500">Email</Label>
              <Input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="contacto@empresa.com" />
            </div>
          </div>
